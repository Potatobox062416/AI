...
北京团队旅行规划单页站点，包含景点索引、四图详情、逐人团队成员资料、分钟级到离京时间、节奏与预算方式、自动行程天数、3—5套行程、自编排程与冲突建议、升旗与开闭馆审查、午晚餐排程、北京名吃分类、美食资料、天气提醒、费用区间和门票预约清单。
...
- 工作区的 `team` 节点显式记录 `memberCount`，并为每位成员记录 `id`、姓名、性别与年龄；旧版顶层 `members` 数据登录后会自动补齐为新结构。
- 成员姓名和年龄未完整通过校验时，只保留最近一次有效名单，不会用半填状态覆盖云端数据。
...
- 美食体验参考涵盖大祥哥来了、大霸子来了、特厨隋卞、食贫道及可核验的其他哔哩哔哩内容；餐馆可同时提供小红书帖子与评论检索入口。地址、人均和营业时间仍由北京文旅或门店事实页核验。

## 数据说明

- 当前景点库含28处景点及112张配图；票价、预约规则与开放口径最新复核日：2026-09-07。北京大学参观规则来自校方保卫部，具体开放日以“参观北大”小程序通知为准。
- 景点索引默认按“国庆综合热度”排序，参考Trip.Best 2026榜单、北京市2025年国庆客流数据和北京旅游网热门清单；搜索和筛选结果沿用同一顺序，不将静态排序表述为实时热度。
...
- 当前启用美食库含42家餐馆及84张配图，可按北京名吃种类、价位和参考来源筛选；新增资料核验于2026-09-08，静态站点不会在每次生成时实时抓取平台或评论区。
